...
class Action:
    ...
    # activat an action, and its consequences will be applied to the world
    def run(self, w=None):
        if w == None:
            w = get_world()
    ...
    # check to see if all the action's requirements are satisfied
    def is_avaliable(self, w=None):
        ...
        if 'actions' in reqs:
            if not all(x in w.player.actions for x in reqs['actions']):
                return False

        if 'worldstate' in reqs:
            if w.worldstate != reqs['worldstate']:
                return False

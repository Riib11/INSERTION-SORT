...
class Location:
    ...
    def get_npc(self, name):
        ...
        if time == "day":
            ...
            npcs = self.night_npcs

        if name in npcs:
            return w.get_npc(name)
...
class Computer(GameObject):
    ...
    # get the path of a file in a computer, as an array, relative to the player's current location in the comptuer
    ...
    # add a file to dictionary that represents the data structure of this computer
    ...
    # see if a path is a directory, and return it
    ...
    # enter a directory (to move around the player's computer location)
    ...
    # open a file (can only open png, txt, and email files)
    ...
    # run a program at the given path
    def run(self, path):
        if self.get_directory(path):
            tp = path.split(".")
            if len(tp) < 2:
    ...
    # ask the player for the password to the computer, and return if it is the correct password
    def prompt_password(self):
        i = input("enter password: ")

        # the cheat code hinted at by the beginning sequence
        if i == "248165":
            return True
